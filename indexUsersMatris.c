...
int leer_datos(float temblores[FILAS][DIAS], int filas, int dias, char nombre_archivo[]) { // Función para leer los datos desde un archivo
...
int calcular_diferencias_consecutivas(float temblores[FILAS][DIAS], int dias) { // Función para calcular las diferencias consecutivas
...
int main() { 
    ...
    printf("Por favor, ingrese el nombre del archivo con los datos (ejemplo: temblores.txt): "); // Mostrar el mensaje de ingreso del nombre del archivo
    ...
    while (!archivo_cargado) { // Mientras no se cargue el archivo
        ...
        if (!archivo_cargado) { // Si no se cargó el archivo
            printf("Por favor, ingrese un nombre de archivo válido: "); // Mostrar el mensaje de error
        }
    }

    // Si se carga correctamente, mostrar los resultados
    mostrar_resultados(temblores, DIAS); // Llamar a la función para mostrar los resultados
